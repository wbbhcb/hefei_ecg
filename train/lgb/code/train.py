...
#     read data
    ...
    data = pd.read_csv(os.path.join(base_path1, 'prob_'+str(i)+'.csv'), header=0, engine='python')
    col1 = [str(i) for i in range(34)] + ['filename']
    data = data[col1]
    col1 = ['prob_'+str(i) for i in range(34)] + ['filename']
    data.columns = col1
    col1 = ['prob_'+str(i) for i in range(34)]
    
    data_2 = pd.read_csv(os.path.join(base_path1, str(i) + '_test.csv'), header=0, engine='python')
    col2 = [str(i) for i in range(feature_len)] + ['filename']
    data_2 = data_2[col2]
    col2 = ['df_'+str(i) for i in range(feature_len)] + ['filename']
    data_2.columns = col2
    col2 = ['df_'+str(i) for i in range(feature_len)]
    
    data = data.merge(data_2, 'left', on='filename')
    
    data_2 = pd.read_csv(os.path.join('ML_feature', str(i) + '_test.csv'), header=0, engine='python')
