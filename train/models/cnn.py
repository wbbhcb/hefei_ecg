import torch.nn as nn
import math
import torch.utils.model_zoo as model_zoo
import torch
import numpy as np


class Res(nn.Module):
    def __init__(self, planes, planes2, stride, kernel_size, i):
        super(Res, self).__init__()
        self.conv3 = nn.Conv1d(planes, planes2, kernel_size=1, stride=1, padding=0)
        self.conv4_1 = nn.Conv1d(planes2, planes2, kernel_size=kernel_size, stride=stride, padding=0)
        self.conv4_2 = nn.Conv1d(planes2, planes2, kernel_size=kernel_size, stride=stride, padding=0)
        self.conv4_3 = nn.Conv1d(planes2, planes2, kernel_size=kernel_size, stride=stride, padding=0)
        self.conv5 = nn.Conv1d(planes2, planes2, kernel_size=1, stride=stride, padding=0)
        self.i = i
        self.maxpool = nn.MaxPool1d(kernel_size=2, stride=2, padding=0)
        self.leakyrelu = nn.LeakyReLU(0.3, inplace=True)
        self.kernel_size = kernel_size
        self.stride = stride

    def mypad(self, x, kernal_size, stride):
        out_size = np.ceil(x.size(2) / stride)
        padded_num = (out_size-1)*stride+kernal_size-x.size(2)
        if padded_num == 0:
            return x
        elif padded_num == 1:
            padded_data2 = torch.zeros((x.size(0), x.size(1), int(padded_num-int(padded_num/2))))
            if torch.cuda.is_available():
                padded_data2 = padded_data2.cuda()
            x = torch.cat((x, padded_data2), 2)
            return x
        else:
            padded_data1 = torch.zeros((x.size(0), x.size(1), int(padded_num/2)))
            padded_data2 = torch.zeros((x.size(0), x.size(1), int(padded_num-int(padded_num/2))))
            if torch.cuda.is_available():
                padded_data1 = padded_data1.cuda()
                padded_data2 = padded_data2.cuda()
            x = torch.cat((padded_data1, x, padded_data2), 2)
            return x

    def forward(self, x):
        if self.i == 0:
            out = self.conv3(x)
        else:
            out = self.conv5(x)
        residual = self.mypad(out, 2, 2)
        residual = self.maxpool(residual)
        out = self.mypad(out, self.kernel_size, self.stride)
        out = self.conv4_1(out)
        out = self.leakyrelu(out)
        out = self.mypad(out, self.kernel_size, self.stride)
        out = self.conv4_2(out)
        out = self.leakyrelu(out)
        
        out = self.mypad(out, self.kernel_size, self.stride)
        out = self.conv4_3(out)
        out = self.leakyrelu(out)
        
        out = self.mypad(out, 2, 2)
        out = self.maxpool(out)
        out = out + residual
        return out


class Resnet_block(nn.Module):
    def __init__(self, inplanes, planes, planes2, kernel_size, stride=1, downsample=None):
        super(Resnet_block, self).__init__()
        self.conv1 = nn.Conv1d(inplanes, planes, kernel_size=1, stride=stride, padding=0)
        self.leakyrelu = nn.LeakyReLU(0.3, inplace=True)
        # nn.Ze
        self.conv2_1 = nn.Conv1d(planes, planes, kernel_size=kernel_size, stride=stride, padding=0)
        self.conv2_2 = nn.Conv1d(planes, planes, kernel_size=kernel_size, stride=stride, padding=0)
        self.maxpool = nn.MaxPool1d(kernel_size=3, stride=3, padding=0)
        self.res1 = Res(planes, planes2, stride, kernel_size, 0)
        self.res2 = Res(planes, planes2, stride, kernel_size, 1)
        self.res3 = Res(planes, planes2, stride, kernel_size, 2)
        self.res4 = Res(planes, planes2, stride, kernel_size, 3)
        self.res5 = Res(planes, planes2, stride, kernel_size, 4)
        self.kernel_size = kernel_size

    def mypad(self, x, kernal_size, stride):
        out_size = np.ceil(x.size(2) / stride)
        padded_num = (out_size-1)*stride+kernal_size-x.size(2)
        if padded_num == 0:
            return x
        elif padded_num == 1:
            padded_data2 = torch.zeros((x.size(0), x.size(1), int(padded_num-int(padded_num/2))))
            if torch.cuda.is_available():
                padded_data2 = padded_data2.cuda()
            x = torch.cat((x, padded_data2), 2)
            return x
        else:
            padded_data1 = torch.zeros((x.size(0), x.size(1), int(padded_num/2)))
            padded_data2 = torch.zeros((x.size(0), x.size(1), int(padded_num-int(padded_num/2))))
            if torch.cuda.is_available():
                padded_data1 = padded_data1.cuda()
                padded_data2 = padded_data2.cuda()
            x = torch.cat((padded_data1, x, padded_data2), 2)
            return x

    def forward(self, x):
        out = self.conv1(x)
        out = self.leakyrelu(out)
        out_ = self.mypad(out, 3, 3)
        residual = self.maxpool(out_)

        # print(residual.size)
        out = self.mypad(out, self.kernel_size, 1)
        out = self.conv2_1(out)
        out = self.leakyrelu(out)
        out = self.mypad(out, self.kernel_size, 1)
        out = self.conv2_2(out)
        out = self.leakyrelu(out)
        out = self.mypad(out, 3, 3)
        out = self.maxpool(out)
        out = out + residual

        out = self.res1(out)
        out = self.res2(out)
        out = self.res3(out)
        out = self.res4(out)
        # out = self.res5(out)
        return out


class CNN(nn.Module):
    def __init__(self, block, inplanes, num_classes=34):
        super(CNN, self).__init__()
        self.globavepool = nn.AdaptiveAvgPool1d(1)
        self.globmaxpool = nn.AdaptiveMaxPool1d(1)
        self.block1 = block(inplanes, 16, 64, 2)
        self.block2 = block(inplanes, 16, 64, 4)
        self.block3 = block(inplanes, 16, 64, 8)
        self.block4 = block(inplanes, 16, 64, 16)
        self.block5 = block(inplanes, 16, 64, 32)
        self.fc1 = nn.Linear(64 * 5,  256)
        self.fc_atten_1 = nn.Linear(64, 64)
        self.fc_atten_2 = nn.Linear(64, 64)
        self.fc_atten_3 = nn.Linear(64, 64)
        self.fc_atten_4 = nn.Linear(64, 64)
        self.fc_atten_5 = nn.Linear(64, 64)
        self.dropout = nn.Dropout(0.3, inplace=True)
        self.fc2 = nn.Linear(512, num_classes)
        # self.sigmoid = nn.Sigmoid()

    def attention(self, x, i):
        out = torch.transpose(x, 1, 2)
        if i == 0:
            out = self.fc_atten_1(out)
        elif i == 1:
            out = self.fc_atten_2(out)
        elif i == 2:
            out = self.fc_atten_3(out)
        elif i == 3:
            out = self.fc_atten_3(out)
        elif i == 5:
            out = self.fc_atten_4(out)
        out = torch.transpose(out, 1, 2)
        weight = self.globavepool(out)
        out = weight * x
        return out

    def forward(self, x):
        out1 = self.block1(x)
        out1 = self.attention(out1,0)
        out2 = self.block2(x)
        out2 = self.attention(out2,1)
        out3 = self.block3(x)
        out3 = self.attention(out3,2)
        out4 = self.block4(x)
        out4 = self.attention(out4,3)
        out5 = self.block5(x)
        out5 = self.attention(out5,4)
        out = torch.cat((out1, out2, out3, out4, out5), 1)
        # out = out2
        # time_step = out.size(2)
        # print(out.size(1))
        out = torch.transpose(out, 1, 2)
        # out = torch.reshape(out, (-1, out.size(2)))
        out = self.fc1(out)
        # out = torch.reshape(out, (-1, time_step, out.size(1)))
        out = torch.transpose(out, 1, 2)
        out1 = self.globavepool(out)
        out2 = self.globmaxpool(out)
        out2 = torch.cat((out1, out2), 1)
        out = torch.reshape(out2, (-1, out2.size(1)))
        out = self.dropout(out)
        out = self.fc2(out)
        # out = self.sigmoid(out)
        # out.backward()
        return out, out2


def myCNN(**kwargs):
    """Constructs a ResNet-34 model.

    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = CNN(Resnet_block, 12, **kwargs)
    return model


# from torchsummary import summary
# device = torch.device("cuda" if torch.cuda.is_available() else "cpu") # PyTorch v0.4.0
# model = CNN(Resnet_block, 12).to(device)
# summary(model, (12, 3613))
