...
#        print('folds: %d, label: %d' % (i, j))
        if use_age_sex == 0:
            save_base_path = os.path.join('../../train/lgb/model', str(i))
        else:
            save_base_path = os.path.join('../../train/lgb/model', str(i))
        # print(save_base_path)
        save_path = os.path.join(save_base_path, 'model_cat' + str(j) + '.pickle')
#        clf = cat.CatBoostClassifier(task_type='GPU', devices='0')
#        clf.load_model(save_path)
        with open(save_path, 'rb+') as f:
            clf = pickle.load(f)
        pred_fold[:, j] = clf.predict_proba(val_data)[:,1]
...
# -----------2
pred2 = np.zeros((len(sub), label_num))
prob_dl2 = np.zeros((len(sub), label_num))
base_path1 = 'DL_feature2'
for i in fold:
    # read data
    print('-----------' + str(i) + '---------------')
    df = sub.copy()
...
#        print('folds: %d, label: %d' % (i, j))
        if use_age_sex == 0:
            save_base_path = os.path.join('../../train/lgb/model2', str(i))
        else:
            save_base_path = os.path.join('../../train/lgb/model2', str(i))
        # print(save_base_path)
        save_path = os.path.join(save_base_path, 'model_cat' + str(j) + '.pickle')
...
#        pred_fold[:, j] = clf.predict(val_data, num_iteration=clf.best_iteration)
    pred2 = pred2 + pred_fold
    
pred2 = pred2 / len(fold)
#print(pred2[0:2,:]-pred[0:2])
pred = (pred2 + pred) / 2
